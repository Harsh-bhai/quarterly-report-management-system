...
const Navbar = () => {
  ...
  return (
    <div>
      <div className="navbar flex flex-col md:flex-row items-center bg-gray-300 md:p-2 shadow-lg py-4 ">
     <div className=' top-0 left-0 absolute m-7 md:m-10 text-3xl font-thin md:hidden'> <GiHamburgerMenu onClick={toggle}  /></div>
        <Link href={"/"}><div className="flex title-font font-medium items-center md:justify-start justify-center text-gray-900 md:z-10 md:my-1 my-0 mx-10 cursor-pointer">
          <img src="/logo1.png" className='w-20' alt="" />
          {/* <span className="ml-3 text-xl">MyShop</span> */}
        </div></Link>
        <div ref={ref} className="z-40   navitems flex flex-col md:flex-row md:items-center md:bg-inherit bg-gray-400 px-10 py-4 md:py-0 absolute top-0 left-0 transform transition-transform -translate-x-full h-full md:h-auto md:w-full w-2/3 md:static md:translate-x-0  md:transition-none ease-in-out" >
          ...
          <div className="flex flex-col md:flex-row md:items-center md:space-x-8  mainitems my-24 md:my-4 space-y-6 md:space-y-0">
            ...
            {/* <Link  href={"/categories"}><div onClick={toggle} className= 'hover:text-gray-600 text-gray-700 cursor-pointer'>Categories</div></Link>
